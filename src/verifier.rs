...
/// Verifier-side fixed data prepared once outside the per-proof verification path.
#[derive(Clone, Debug, PartialEq)]
pub struct PreparedVerifierInput {
    domain: PlonkDomain,
    transcript_input: TranscriptPreprocessedInput,
}

impl PreparedVerifierInput {
    /// Return the committed fixed input bound into the transcript.
    pub fn transcript_input(&self) -> &TranscriptPreprocessedInput {
        &self.transcript_input
    }
}

/// Prepare the fixed verifier data once so benchmark code can time preprocessing separately.
pub fn prepare_verifier_input(
    verifier_input: &VerifierPreprocessedInput,
    srs: &KzgSrs,
) -> Result<PreparedVerifierInput> {
    let domain = build_and_validate_domain(verifier_input)
        .ok_or(PlonkError::InvalidInput("verifier domain parameters are inconsistent"))?;
    let transcript_input = verifier_input.to_transcript_preprocessed_input(srs)?;
    Ok(PreparedVerifierInput {
        domain,
        transcript_input,
    })
}
...
pub fn verify(
    ...
    let prepared_input = match prepare_verifier_input(verifier_input, srs) {
        Ok(input) => input,
        Err(_) => return Ok(false),
    };
    verify_with_prepared_input(proof, public_inputs, verifier_input, &prepared_input, srs)
}

/// Verify a proof with fixed commitments already prepared outside the per-proof path.
pub fn verify_with_prepared_input(
    proof: &PlonkProof,
    public_inputs: &[Fr],
    verifier_input: &VerifierPreprocessedInput,
    ...
    if prepared_input.transcript_input.domain != verifier_input.domain {
        return Ok(false);
    }
    if prepared_input.transcript_input.protocol_params != verifier_input.protocol_params {
        return Ok(false);
    }

    if !validate_verifier_inputs(public_inputs, verifier_input, &prepared_input.domain) {
        return Ok(false);
    }

    let challenges =
        Transcript::default().replay_phase_9_proof(proof, public_inputs, prepared_input.transcript_input());
    ...
    let linearization_commitment = build_linearization_commitment(
        proof,
        verifier_input,
        prepared_input.transcript_input(),
        &prepared_input.domain,
        public_inputs,
        challenges.alpha,
        challenges.beta,
        challenges.gamma,
    ...
    let shifted_zeta = prepared_input.domain.group_gen() * challenges.zeta;
    ...
    // Paper form: left_group = zeta*W_z + u*(omega*zeta)*W_{z omega} + F - E.
